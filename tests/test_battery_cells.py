...
class TestBatteryCellReadingModel:
    ...
    def test_from_cell_voltages_empty_list(self):
        ...
        reading = BatteryCellReading.from_cell_voltages(timestamp=datetime.now(timezone.utc), cell_voltages=[])
        assert reading is None

    def test_from_cell_voltages_all_none(self):
        """Test that all-None voltage list returns None."""
        reading = BatteryCellReading.from_cell_voltages(timestamp=datetime.now(timezone.utc), cell_voltages=[None] * 96)
        assert reading is None
    ...
    def test_to_dict(self):
        ...
        voltages = [3.8] * 96
        reading = BatteryCellReading.from_cell_voltages(
            timestamp=datetime.now(timezone.utc), cell_voltages=voltages, state_of_charge=75.0
        )
...
class TestBatteryCellEndpoints:
    ...
    def test_add_cell_reading(self, client, db_session):
        ...
        response = client.post(
            ...
            json={"cell_voltages": [3.8] * 96, "state_of_charge": 80.0, "ambient_temp_f": 70.0},
        )
    ...
    def test_add_cell_reading_no_voltages(self, client, db_session):
        ...
        response = client.post("/api/battery/cells/add", json={"state_of_charge": 80.0})
    ...
    def test_add_cell_reading_empty_voltages(self, client, db_session):
        ...
        response = client.post("/api/battery/cells/add", json={"cell_voltages": []})
    ...
    def test_get_latest_cell_reading(self, client, db_session):
        ...
        # Add a reading first
        client.post("/api/battery/cells/add", json={"cell_voltages": [3.8] * 96})
    ...
    def test_get_cell_readings_with_limit(self, client, db_session):
        ...
        # Add 3 readings
        for i in range(3):
            client.post("/api/battery/cells/add", json={"cell_voltages": [3.7 + i * 0.1] * 96})
    ...
    def test_get_cell_analysis_with_data(self, client, db_session):
        ...
        voltages = [3.7] * 32 + [3.8] * 32 + [3.9] * 32
        client.post("/api/battery/cells/add", json={"cell_voltages": voltages})
    ...
    def test_add_cell_reading_with_timestamp(self, client, db_session):
        ...
        response = client.post(
            "/api/battery/cells/add", json={"cell_voltages": [3.8] * 96, "timestamp": "2024-01-15T10:30:00Z"}
        )
    ...
    def test_add_cell_reading_invalid_timestamp(self, client, db_session):
        ...
        response = client.post(
            "/api/battery/cells/add", json={"cell_voltages": [3.8] * 96, "timestamp": "not-a-timestamp"}
        )
