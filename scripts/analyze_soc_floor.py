...
def analyze_soc_data(transitions: List[dict]) -> dict:
    ...
    analysis["temperature_analysis"] = {
        ...
        "hot_above_80f": {"count": len(hot_socs), "avg_soc": round(statistics.mean(hot_socs), 2) if hot_socs else None},
    }
    ...
    if len(transitions) >= 10:
        ...
        for i in range(4):
            ...
            if quarter_socs:
                quarters.append(
                    {"quarter": i + 1, "count": len(quarter_socs), "avg_soc": round(statistics.mean(quarter_socs), 2)}
                )
