...
def detect_operating_mode(
    ...
    motor_a_active = bool(motor_a_rpm and motor_a_rpm > MOTOR_ACTIVE_THRESHOLD)
    motor_b_active = bool(motor_b_rpm and motor_b_rpm > MOTOR_ACTIVE_THRESHOLD)
    motor_b_significant = bool(motor_b_rpm and motor_b_rpm > MOTOR_B_SIGNIFICANT_THRESHOLD)
    generator_active = bool(generator_rpm and generator_rpm > GENERATOR_ACTIVE_THRESHOLD)
    engine_active = bool(engine_rpm and engine_rpm > ENGINE_ACTIVE_THRESHOLD)
    battery_charging = bool(hv_battery_power_kw and hv_battery_power_kw <= -1.0)

    return _classify_mode(motor_a_active, motor_b_active, motor_b_significant,
                          generator_active, engine_active, battery_charging)


def _classify_mode(motor_a_active, motor_b_active, motor_b_significant,
                   generator_active, engine_active, battery_charging):
    """Classify powertrain operating mode from component states."""
    ...
    if motor_a_active and engine_active and generator_active:
        return PowertrainMode.MOUNTAIN_MODE if battery_charging else PowertrainMode.HOLD_MODE
