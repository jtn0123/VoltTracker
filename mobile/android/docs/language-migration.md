...
are loaded on demand by injecting a classic `<script>` (`core.ts` `loadDashboardScript`).
...
| 2026-06-05 | T2b Drive slice: `drive` moved to a `.ts` module. The public `VoltDashboard` render helpers stay attached for `telemetry.ts`, while the chip model, chart points, DOM widths, canvas node, and resize debounce state now use TS annotations. |
...
| 2026-06-05 | Runtime dogfood found Android 9 WebView 66 could not parse the `es2022` dashboard bundle (`Unexpected token ?`). Lowered esbuild output to `chrome66` and added a shipped-bundle syntax guard so desktop Chromium tests cannot mask unsupported WebView syntax. |
