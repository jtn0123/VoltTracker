...
| Resource loads | Scripts, styles, images, and tile fetches. | CSP meta (`dashboard-src/index.template.html`) allows only first-party scripts and styles — `script-src 'self'; style-src 'self'`, no `unsafe-*` tokens anywhere — plus the documented tile hosts for remote map tile images/fetches. `style-src` needs no `'unsafe-inline'`: Leaflet positions tiles/markers through CSSOM (`el.style.prop = ...`), which `style-src` does not restrict, and the app's own markup/code carries no `style=""` attributes or injected `<style>` blocks (map legend swatches and drive-event `divIcon` markers use `data-*` attributes styled by `screens-map.css`). If a Leaflet upgrade or new code reintroduces parsed style attributes (e.g. `style=""` inside HTML strings passed to `divIcon`/popups, or `setAttribute("style", ...)`), the affected element renders unstyled under this policy — fix the markup to use classes/CSSOM rather than re-adding `'unsafe-inline'`. |
