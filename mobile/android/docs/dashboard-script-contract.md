...
`dashboard-tests/script-order.test.js` asserts two things: (1) `index.template.html` and the
generated `index.html` load `js/app.js` as a single classic (non-module) script, and (2)
`build.mjs`'s `EAGER` array matches the dependency order above. When adding a new eager script,
add it to `dashboard-src/js/`, insert it into `EAGER` in `build.mjs`, and update the test's
