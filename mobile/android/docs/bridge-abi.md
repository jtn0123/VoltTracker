...
| `getStorageDetails()` | — | `String` (JSON object) | — | Synchronous lazy detail read; calls `MainActivity.getStorageDetailsJson()`. Returns `{storageDetails:true, latestReview, latestRoute, recentRoutes, overview, chargeSummary, batterySummary, latestVehicle, enhancedCapabilities, detailedSignalCatalog}` and is requested by the dashboard after the overview renders. `batterySummary.latestCellSnapshot` carries the newest 96-cell probe result (`{capturedAtMs, cellCount, cells:[{index, voltage}, …]}`, `index` 1-based; `{}` until a cell probe has run) for the per-cell voltage map. |
...
| `shareTripCard(cardJson)` | `String` (JSON object) | `String` (JSON object) | `safe(cardJson, 4096)`; every drawn field re-clamped (≤ 64 chars, ≤ 6 stats) | Shareable drive-summary card: renders the drive's route outline plus the trip-detail sheet's already-formatted stat strings (`{routeKey, title, subtitle, stats:[{label, value}, …]}` — units/cost math stays single-sourced in the WebView) as a 1080×1080 PNG (`TripShareCardRenderer`), records the export, and launches the OS share sheet (`image/png`) via the same trip-exports FileProvider path. Rides the `MainActivity.exportTripFromBridge(cardJson, "card")` host seam (the `card` sentinel dispatches to `TripExportController.shareTripCard` — no new host override). Returns `{ok:true, format:"card", fileName, bytes, pointCount:0}` or `{ok:false, error, message}` (e.g. `invalid_card`, `invalid_id`, `storage_unavailable`, `export_failed`, `share_failed`). |
...
| `detailProbeLast(stage)` | `String` | void | `safe(stage, 32)` then `normalizeStage`; bounds applied to cached device values | Runs a Detail Probe for the given enhanced-PID stage against the remembered/candidate adapter (`ACTION_TPMS_SCAN`); `publishStatus("blocked", …)` if no valid remembered adapter. The `"cells"` stage runs the dedicated 96-cell voltage snapshot pass (`CellVoltageProbeRunner`) instead of a catalog sweep. |
...
| `backfillTelemetry(json)` | `String` (JSON object) | `{samples: [<telemetry sample>, …]}` — samples broadcast while the Activity was paused, buffered in `LiveDashboardSnapshot`'s bounded ring (≤300, oldest first) and sliced to those newer than the pre-pause `updatedAt`. Replayed into the chart histories / session distance / live route only; never advances the live tiles or the staleness clock. The freshest sample is delivered via `updateTelemetry` instead and is excluded from the batch. | `LiveDashboardStatePublisher.publish` on resume / dashboard handshake | `VD.backfillTelemetry` (`telemetry.ts`) |
