...
| `clearStoredData()` | — | void | native confirmation; refuses while logging is active | Runs `ObdLocalStore.clearAllData()` on a background executor (one DELETE per table — 15 tables — in one transaction). Reports status via `publishStatus`. |
