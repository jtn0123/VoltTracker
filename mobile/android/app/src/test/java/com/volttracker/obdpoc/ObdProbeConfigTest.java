...
/**
 * Guards the probe lists in {@link ObdService} against the class of bug that has cost
...
public class ObdProbeConfigTest {
    ...
    public void liveProbesAreAtCommandsOrModeOnePids() {
        for (String probe : ObdService.LIVE_PROBES) {
    ...
    public void protocolProbesAreAtspCommands() {
        for (String probe : ObdService.PROTOCOL_PROBES) {
    ...
    public void capabilityProbesAreModeOnePids() {
        for (String probe : ObdService.CAPABILITY_PROBES) {
    ...
    public void correctedPackVoltagePidIsConfigured() {
        ...
        assertTrue("HV pack voltage PID 222429 missing", contains(ObdService.VOLT_7E1_PROBES, "222429"));
        assertTrue("standard SOC PID 015B missing from live probes",
                contains(ObdService.LIVE_PROBES, "015B"));
    }

    private static String[] allVoltProbes() {
        String[] a = ObdService.VOLT_7E1_PROBES;
        String[] b = ObdService.VOLT_7E4_PROBES;
