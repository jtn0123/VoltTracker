  ...
  function renderMap() {
    ...
    setText("mapPointBadge", `${points.length} pts`);
    const routeSession = sessionForRoute(route);
    setText("mapTitle", hasRoute ? `${formatDistance(route.distanceMeters || 0)} route` : "No route recorded yet");
    setText("mapKicker", hasRoute ? `GPS map - ${formatWhen(routeSession.startedAtMs)}` : "GPS map");
    setText("mapDistance", hasRoute ? formatDistance(route.distanceMeters || 0) : "--");
    const session = route.session || {};
    const duration = Number(session.endedAtMs || Date.now()) - Number(session.startedAtMs || 0);
    setText("mapDuration", hasRoute && duration > 0 ? formatDuration(duration) : "--");
    const avgAccuracy = average(points.map((p) => Number(p.accuracyM)).filter(Number.isFinite));
    setText("mapAccuracy", avgAccuracy ? `~${Math.round(avgAccuracy)}m GPS` : "--");
  ...
  function drawMapRoute(points, hasRoute, layer, routeSession) {
    ...
    detectStops(points).slice(0, 20).forEach((stop) => {
      ...
      }).bindTooltip(`Stop · ${formatDuration(stop.durationMs)}`).addTo(mapLayerGroups.stops);
    });
  ...
  function renderMapSessionList(routes) {
    ...
    list.innerHTML = routes.length
      ? routes.map((r) => {
        ...
        return `
        <button type="button" class="history-row ${active ? "is-active" : ""}" data-map-session="${escapeHtml(s.id || "")}">
          <span><strong>${escapeHtml(s.mode || "session")} - ${escapeHtml(s.adapterName || "OBD adapter")}</strong><small>${formatWhen(s.startedAtMs)} - ${formatDistance(Number(r.distanceMeters || 0))} - ${Number(r.pointCount || 0)} pts</small></span>
          <b>${s.status || "stored"}</b>
        </button>`;
      }).join("")
      : '<p class="status-copy">No route-bearing SQLite sessions yet. Start a real logged drive with GPS permission and the route will render here.</p>';
  }
  ...
  function loadSampleData() {
    ...
    renderRealTrips();
    renderInsightStats();
    setStatus({ state: "ready", detail: "Preview sample drive loaded (real logged route)." });
  }
